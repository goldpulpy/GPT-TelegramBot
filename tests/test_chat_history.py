...
class TestChatHistory(unittest.TestCase):
    ...
    def test_ensure_chat_history_limit(self) -> None:
        ...
        self.assertEqual(
            self.chat_history.chat_history[0], {"message": "Test 2"}
        )
        self.assertEqual(
            self.chat_history.chat_history[1], {"message": "Test 3"}
        )

    @patch('app.utils.chat_history.datetime')
    def test_clear_expired_chat_history(self, mock_datetime) -> None:
    ...
    @patch('app.utils.chat_history.datetime')
    def test_reset_clear_time(self, mock_datetime) -> None:
        mock_datetime.now.return_value = datetime(2020, 1, 1, 12, 0, 0)
        self.chat_history.reset_clear_time()
        self.assertEqual(
            self.chat_history.clear_time, datetime(2020, 1, 1, 12, 30, 0)
        )
